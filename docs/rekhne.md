...
On September 2, 2026, a new shallow futility candidate was opened on top of frozen H3b. It prunes only non-root, non-PV, not-in-check, non-TT, non-killer quiets from move five onward after a static fail-low margin test, and it skips pruning when the quiet move gives check. The local depth-5 benchmark changed from `165402 / 485491effa74cdce` to `119848 / ac94e5d6a5c02a46`. A short equal-node `10000`-node screen then scored `21.5/40` (53.75%, reported `+26.11 +/-97.12 Elo`) against frozen H3b, and a short equal-time `0.5+0.005` screen scored `24.5/40` (61.25%, reported `+79.53 +/-79.64 Elo`). These are preliminary local screens only; retention still requires a larger timed test.
