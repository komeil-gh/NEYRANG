...
fn shallow_futility_move_is_eligible(
    move_index: usize,
    mv: Move,
    tt_move: Option<Move>,
    killers: [Move; 2],
...
fn shallow_futility_bound_allows(eval: Option<i32>, gives_check: bool, alpha: i32) -> bool {
    !gives_check && eval.is_some_and(|value| value + SHALLOW_FUTILITY_MARGIN <= alpha)
}
...
impl<'a> Searcher<'a> {
    ...
    fn negamax(
        ...
        while let Some(mv) = picker.next_move(position, &self.history) {
            ...
            let can_prune = futility_eligible
                && shallow_futility_move_is_eligible(
                    move_index,
                    mv,
                    tt_move,
                    self.killers[ply],
                    self.history.score(moving_color, mv),
                );
            if can_prune && futility_eval.is_none() {
                futility_eval = Some(self.evaluate_position(position, ply));
            }
            ...
            if can_prune && shallow_futility_bound_allows(futility_eval, gives_check, alpha) {
...
mod tests {
    ...
    #[test]
    fn shallow_futility_move_guards_protect_special_moves() {
        ...
        assert!(super::shallow_futility_move_is_eligible(
        ...
        assert!(super::shallow_futility_bound_allows(
            Some(-1_000),
            false,
            1_000,
        ));
        ...
        assert!(!super::shallow_futility_bound_allows(None, false, 1_000,));
    }
