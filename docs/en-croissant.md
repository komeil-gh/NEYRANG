...
```text
.../dist/en-croissant/neyrang-0.2.0-macos-arm64
```

In En Croissant 0.15, open **Engines**, choose the local-engine add action, and select that executable. En Croissant probes it with `uci` and `isready`; the detected identity should be `NEYRANG 0.2.0`, with `Hash`, `Threads`, and `Move Overhead` settings.
...
The repository includes a paired smoke-match PGN at `examples/neyrang-vs-stockfish-smoke.pgn`. Open it directly from En Croissant or run:
...
The harness runs the first five EPD openings twice with colors reversed. By default NEYRANG searches to depth 3 and Stockfish to depth 1. These deliberately unequal settings exercise UCI and legal complete-game behavior; they are not a strength comparison or Elo evidence.
