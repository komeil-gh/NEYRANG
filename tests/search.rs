...
#[cfg(feature = "stats")]
#[test]
fn null_move_pruning_stays_disabled_in_pawn_only_endgames() {
    let mut position = Position::from_fen("4k3/8/8/4K3/4P3/8/8/8 w - - 0 1")
        .expect("pawn-only NMP fixture must be valid");
    let hashes = [position.hash()];
    let stop = AtomicBool::new(false);
    let mut searcher = Searcher::new(&stop);
    ...
    assert_eq!(result.statistics.null_move_attempts, 0);
    assert_eq!(result.statistics.null_move_cutoffs, 0);
}

#[cfg(feature = "stats")]
#[test]
fn node_stop_during_selective_search_restores_the_root_position() {
    let mut position = Position::startpos();
