...
#[test]
fn compare_corpus_emits_reproducible_machine_readable_paired_evidence() {
    ...
    fs::write(&candidate_path, constant_network(0).to_bytes()).unwrap();
    fs::write(&corpus_path, encode_games(&[draw_game()]).unwrap()).unwrap();

    let first = Command::new(env!("CARGO_BIN_EXE_compare-corpus"))
        .arg(&baseline_path)
        .arg(&candidate_path)
        .arg(&corpus_path)
        .arg("2026090116000001")
        .arg("1000")
        .output()
        .unwrap();
    let repeat = Command::new(env!("CARGO_BIN_EXE_compare-corpus"))
        .arg(&baseline_path)
        .arg(&candidate_path)
        .arg(&corpus_path)
        .arg("2026090116000001")
    ...
    assert!(stdout.contains("\"schema\":\"neyrang-nnue-paired-diagnostics-v1\""));
    assert!(stdout.contains("\"games\":1"));
    assert!(stdout.contains("\"positions\":1"));
    assert!(stdout.contains("\"improved_games\":1"));
    assert!(stdout.contains("\"bootstrap_seed\":2026090116000001"));
...
fn unique_temp_directory() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "neyrang-nnue-compare-corpus-{}-{nonce}",
        std::process::id()
    ))
}
